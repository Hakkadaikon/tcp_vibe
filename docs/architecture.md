...
| TUN | カーネルの TUN デバイス (L3) | IP の配送はカーネル (TCP は通さない) | root | 実機 |
| AF_PACKET | NIC への raw バイト I/O (L2) | 使わない (Ethernet/IP/ARP を自作) | CAP_NET_RAW | 実機 |

カーネルへの依存は三段階に分かれる。

第一は、カーネルのネットワークプロトコルを一切通さない手段である。
メモリ仮想リンクと AF_PACKET がこれにあたる。
前者はプロセス内に閉じ、後者は NIC への生バイト出力だけをカーネルに任せて Ethernet と IP と ARP を自作スタックが組む。
AF_PACKET は、実機で IP も ARP も含めて完全に自作スタックが処理する本命である。

第二は、運搬の土管だけカーネルを借りる手段である。
Unix domain socket と UDP トンネルがこれにあたり、自作スタックが組んだ IP パケットをペイロードとして運ぶだけで、TCP/IP のロジックは通さない。
特権が要らないため、root のない環境でも別プロセス間の実通信を確かめられる。

第三は、TUN である。
TUN は TCP を自作スタックが処理する一方、IP パケットの配送はカーネルの L3 に任せる。
実機の本物のネットワークに L3 で出せるうえに、ARP も含めて IP より下を自分で組まずに済むため手軽だが、IP 層はカーネルが受け持つ点で第一の手段とは異なる。
