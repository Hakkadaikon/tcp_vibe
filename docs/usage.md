...
あとの手順で iptables による RST 抑止を挟むのはこのためである。

実機でカーネルのプロトコルを一切使わない完全自作の通信をするなら、IP も ARP も自作スタックが組む AF_PACKET を選ぶ。
ただし AF_PACKET は CAP_NET_RAW が要り、同一 L2 セグメント上での通信を前提とするため、TUN ほど手軽ではない。
ここでは手軽な TUN の手順を示す。
