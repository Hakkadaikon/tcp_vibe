...
## P12: レイヤードアーキテクチャ適用(3パッケージ分割)

ゴール: `package tcp` フラット28ファイルを依存一方向の3レイヤーへ再配置。
振る舞い保存(ロジック不変のファイル移動 + 最小 export 化)。粒度はユーザー確認済=3パッケージ。

レイヤー(依存方向: transport → link → network、transport → network):
- tcp/network/ : ipv4.go, checksum.go, bytes.go + debug.go の debugf/ipStr。
  export化: tcpSegment→TCPSegment、be16等→Be16等
- tcp/link/    : afpacket_linux/arp/tun_linux/udplink/unixlink/link/holepunch/rendezvous。
  Link interface・ErrLinkClosed・pipeLink/NewPipeLink を保持。network.* 参照
- tcp/transport/ : TCP本体16ファイル。Clock を link から移設。flagsStr→Flags.String()。
  link.* / network.* 参照

前提作業: flagsStr→Flags.String() リファクタ / 白箱テスト各パッケージへ再配置 /
export化シンボルの全参照置換 / cmd・e2e の import 2系統化。

検証ゲート(振る舞い保存): 各ステップ後 go build 緑、go test ./... 緑(ベースライン tcp ok 2.7s)、
go test -tags e2e 緑、go vet クリーン。新規 TLA+/Lean 不要(ロジック不変、既存 loopeng/fv が設計担保)。

手順(各 micro-commit、ビルド緑維持):
- [x] S1: network パッケージ作成(ipv4/checksum/bytes/debugf+ipStr、export化)
- [x] S2: flagsStr → Flags.String() リファクタ
- [x] S3: link パッケージ作成(8ファイル、network参照に置換)
- [x] S4: transport パッケージ作成(残り、Clock移設、link/network参照)
- [x] S5: cmd/ の import 更新(tcpdemo→transport、rendezvous→link)
- [x] S6: テスト全再配置・全緑確認・vet・race・e2e・docs

## レビュー欄(P12)

完了。`tcp/` フラット28ファイル → `tcp/network`(4src) / `tcp/link`(8src) / `tcp/transport`(17src)
の3パッケージへ。依存は一方向(transport → link → network、transport → network)、循環なし。

- 振る舞い保存リファクタ。ロジック・コメントは変えずパッケージ境界とファイル位置のみ変更。
  新規 TLA+/Lean は不要と判断(状態遷移設計を変えないため)。安全網は既存テスト45本。
- 検証ゲート全通過: go build / go vet クリーン / go test 全緑 / go test -race -count=3 緑 /
  go test -tags e2e 緑 / gofmt -l 空 / fuzz seed corpus 緑。
- export 化したのは層境界を越えるシンボルのみ(network.Be16 等、TCPSegment、Debugf/IPStr)。
  transport 内部の TCB 白箱結合は1パッケージに封じ込めたまま壊さず。
- flagsStr は Flags.String() stringer へ昇格(transport 型依存を debug から除去)。
- Clock は link でなく transport へ(利用者が transport のみのため)。
- 白箱テストは各層へ再配置。link/transport 両方を触る loopback 結合テストは transport に残し
  link の公開コンストラクタ越しに駆動。
- docs/architecture.md にパッケージ構成と依存方向を反映。
