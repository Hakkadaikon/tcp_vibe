...
- リンク層の抽象化：カーネル依存の度合いが異なる複数のリンクを同じインタフェースの背後に置き、差し替え可能にする。テスト用のメモリ仮想リンク、特権の要らない Unix domain socket リンクと UDP トンネル、実機用の TUN ドライバと AF_PACKET ドライバを揃える。
- Unix domain socket リンク：自作スタックが組み立てた IP パケットを AF_UNIX のデータグラムとしてそのまま運ぶ。カーネルの UDP/IP プロトコルすら通さない別プロセス間のバイト土管で、raw socket でないため特権を要さない。
- ARP の自作 (RFC 826)：宛先 IP から MAC を、カーネルに頼らず自作スタックで解決する。これを AF_PACKET ドライバに組み込み、Ethernet と IP と ARP のすべてを自作スタックが組み立てる。AF_PACKET ドライバは CAP_NET_RAW を要し、同一 L2 セグメント上での通信を前提とする。
...
- `unixlink.go`：Unix domain socket を IP パケットの土管として使うドライバ。UDP/IP プロトコルを通さず、特権も要さない。
- `udplink.go`：UDP ソケットを IP パケットの土管として使うドライバ。特権を要さず、自作スタック同士の実通信を root 無しで実演できる。
- `cmd/tcpdemo`：TUN、UDP トンネル、Unix domain socket のいずれか越しに握手と close を実演するデモ。
...
- AF_PACKET ドライバは Ethernet フレームの組み立てと ARP (RFC 826) を自作スタックで行いますが、同一 L2 セグメント上での通信を前提とし、gateway を越えるサブネット外への解決は扱いません。proxy ARP や gratuitous ARP も対象外です。
...
これを避けるため、IP パケットを運ぶ土管として、UDP ソケットを使うリンク (`--link=udp`) と Unix domain socket を使うリンク (`--link=unix`) を用意しています。
どちらも raw socket でないため特権を要さず、root のないサンドボックスでも動きます。
`--link=unix` では運搬先をパスで指定します (`--unix-local <path>`、`--unix-remote <path>`)。
...
Unix domain socket リンクも運搬層が違うだけで、扱いは同じです。
